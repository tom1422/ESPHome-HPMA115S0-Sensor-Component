...
void HPMA115S0Component::setupLate() {
  while (setupTries < 3) { // Loop 3 times in case sensor had a malfunction
    ...
    if (stop_autosend()) {
      if (start_measurement()) {
        launchSuccess = true;
        ESP_LOGI(TAG, "Found sensor successfully.");
        break;
      }
    }
  }
  if (!launchSuccess) ESP_LOGI(TAG, "HPMA115S0 sensor was not found.");
}
...
void HPMA115S0Component::update() {
  if (millis() > (startupDelay + setupTime)) {
    setupLate();
  }
  if (launchSuccess) {
